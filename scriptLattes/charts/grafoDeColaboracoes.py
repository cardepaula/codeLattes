...
class GrafoDeColaboracoes:
    ...
    def criarGrafoDeCoAutoriaSemPesos(self):
        ...
        grafo = pygraphviz.AGraph(
            directed=False, overlap="False", id="grafo1", name="grafo1"
        )
        ...
        for m in range(0, self.grupo.numeroDeMembros()):
            ...
            nome = (
                self.abreviarNome(membro.nomeCompleto)
                + " ["
                + str(int(self.grupo.vetorDeCoAutoria[m]))
                + "]"
            )
            if self.grupo.vetorDeCoAutoria[m] > 0 or self.grupo.obterParametro(
                ...
                try:
                    grafo.add_node(
                        ...
                        height="0.2",
                        URL="membro-" + membro.idLattes + ".html",
                    )
    ...
    def criarGrafoDeCoAutoriaComPesos(self):
        ...
        grafo = pygraphviz.AGraph(
            directed=False, overlap="False", id="grafo2", name="grafo2"
        )
        grafo.node_attr["shape"] = "rectangle"
        grafo.node_attr["fontsize"] = "8"
        grafo.node_attr["style"] = "filled"
            ...
            for j in range(i, self.grupo.numeroDeMembros()):
                if self.grupo.matrizDeAdjacencia[i, j] > 0:
                    grafo.add_edge(
                        i,
                        j,
                        label=str(self.grupo.matrizDeAdjacencia[i, j]),
                        fontsize="8",
                    )
    ...
    def criarGrafoDeCoAutoriaNormalizada(self):
        ...
        grafo = pygraphviz.AGraph(
            directed=True, overlap="False", id="grafo3", name="grafo3"
        )
        grafo.node_attr["shape"] = "rectangle"
        grafo.node_attr["fontsize"] = "8"
        grafo.node_attr["style"] = "filled"
    ...
    def criarGrafoDeCoAutoriaCompleta(self):
        ...
        for m in range(0, self.grupo.numeroDeMembros()):
            membro = self.grupo.listaDeMembros[m]
            nome = (
                self.abreviarNome(membro.nomeCompleto).encode("utf8")
                + " ["
                + str(int(self.grupo.vetorDeCoAutoria[m]))
                + "]"
            )
