...
class GeradorDeXML:
    ...
    def gerarXmlParaGrupo(self):
        ...
        xml += (
            '<curriculo_lattes data_processamento="'
            + self.getDataProcessamento()
            + '">\n'
        )
        for registro in self.membros:
            ...
            try:
                ...
                xmlTemp += self.getOutrosProjetos(registro)
    ...
    def getListaOCOutroTipoDeOrientacao(self, registro):
        ...
        if registro.listaOCOutroTipoDeOrientacao:
            ...
            for orientacao in registro.listaOCOutroTipoDeOrientacao:
                ...
                xmlTemp += (
                    "          <nome_aluno>" + orientacao.nome + "</nome_aluno>\n"
                )
                xmlTemp += (
                    "          <titulo_trabalho>"
                    + orientacao.tituloDoTrabalho
                    + "</titulo_trabalho>\n"
                )
                xmlTemp += "          <ano>" + str(orientacao.ano) + "</ano>\n"
                xmlTemp += (
                    "          <instituicao>"
                    + orientacao.instituicao
                    + "</instituicao>\n"
                )
                xmlTemp += (
                    "          <agencia_de_fomento>"
                    + orientacao.agenciaDeFomento
                    + "</agencia_de_fomento>\n"
                )
                xmlTemp += (
                    "          <tipo_de_orientacao>"
                    + orientacao.tipoDeOrientacao
                    + "</tipo_de_orientacao>\n"
                )
                xmlTemp += "        </orientacao_outra>\n"
            xmlTemp += "    </orientacao_outros_tipos_concluido>\n"
    ...
    def getListaApresentacaoDeTrabalho(self, registro):
        ...
        if registro.listaApresentacaoDeTrabalho:
            ...
            for apresentacao in registro.listaApresentacaoDeTrabalho:
                ...
                xmlTemp += (
                    "          <natureza>" + apresentacao.natureza + "</natureza>\n"
                )
                xmlTemp += "        </trabalho_apresentado>\n"
            xmlTemp += "    </apresentacao_trabalho>\n"
        return xmlTemp
    ...
    def getListaResumoEmCongresso(self, registro):
        ...
        if registro.listaResumoEmCongresso:
            ...
            for resumo in registro.listaResumoEmCongresso:
                ...
                xmlTemp += (
                    "          <nome_evento>" + resumo.nomeDoEvento + "</nome_evento>\n"
                )
    ...
    def getListaTrabalhoCompletoEmCongresso(self, registro):
        ...
        if registro.listaTrabalhoCompletoEmCongresso:
            ...
            for trabalho_completo in registro.listaTrabalhoCompletoEmCongresso:
                ...
                xmlTemp += (
                    "          <autores>" + trabalho_completo.autores + "</autores>\n"
                )
                xmlTemp += (
                    "          <titulo>" + trabalho_completo.titulo + "</titulo>\n"
                )
                xmlTemp += (
                    "          <nome_evento>"
                    + trabalho_completo.nomeDoEvento
                    + "</nome_evento>\n"
                )
                xmlTemp += "          <ano>" + str(trabalho_completo.ano) + "</ano>\n"
                xmlTemp += (
                    "          <volume>" + trabalho_completo.volume + "</volume>\n"
                )
                xmlTemp += (
                    "          <paginas>" + trabalho_completo.paginas + "</paginas>\n"
                )
                xmlTemp += "        </trabalho_completo>\n"
            xmlTemp += "    </trabalho_completo_congresso>\n"
        return xmlTemp
    ...
    def getListaLivroPublicado(self, registro):
        ...
        if registro.listaLivroPublicado:
            ...
            for livro_publicado in registro.listaLivroPublicado:
                xmlTemp += "        <livro>\n"
                xmlTemp += (
                    "          <autores>" + livro_publicado.autores + "</autores>\n"
                )
                ...
                xmlTemp += (
                    "          <paginas>" + livro_publicado.paginas + "</paginas>\n"
                )
    ...
    def getListaArtigosEmPeriodicos(self, registro):
        ...
        if registro.listaArtigoEmPeriodico:
            ...
            for artigos_periodicos in registro.listaArtigoEmPeriodico:
                ...
                xmlTemp += (
                    "          <titulo>" + artigos_periodicos.titulo + "</titulo>\n"
                )
                xmlTemp += (
                    "          <autores>" + artigos_periodicos.autores + "</autores>\n"
                )
                xmlTemp += (
                    "          <revista>" + artigos_periodicos.revista + "</revista>\n"
                )
                xmlTemp += (
                    "          <volume>" + artigos_periodicos.revista + "</volume>\n"
                )
                xmlTemp += (
                    "          <paginas>" + artigos_periodicos.paginas + "</paginas>\n"
                )
                xmlTemp += (
                    "          <numero>" + artigos_periodicos.numero + "</numero>\n"
                )
    ...
    def getDadosIdentificacao(self, registro):
        ...
        xmlTemp += (
            "      <identificador10>"
            + registro.identificador10
            + "</identificador10>\n"
        )
        xmlTemp += "      <nome_inicial>" + registro.nomeInicial + "</nome_inicial>\n"
        xmlTemp += (
            "      <nome_completo>" + registro.nomeCompleto + "</nome_completo>\n"
        )
        xmlTemp += (
            "      <nome_citacao_bibliografica>"
            + registro.nomeEmCitacoesBibliograficas
            + "</nome_citacao_bibliografica>\n"
        )
        xmlTemp += "      <sexo>" + registro.sexo + "</sexo>\n"
        xmlTemp += "    </identificacao>\n"
        return xmlTemp
    ...
    def getDadosEndereco(self, registro):
        ...
        xmlTemp += (
            ...
            + registro.enderecoProfissional
            + "</endereco_profissional>\n"
        )
        xmlTemp += (
            "      <endereco_profissional_lat>"
            + registro.enderecoProfissionalLat
            + "</endereco_profissional_lat>\n"
        )
        xmlTemp += (
            "      <endereco_profissional_long>"
            + registro.enderecoProfissionalLon
            + "</endereco_profissional_long>\n"
        )
        xmlTemp += "    </endereco>\n"
        return xmlTemp
    ...
    def getFormacaoAcademica(self, registro):
        ...
        if registro.listaFormacaoAcademica:
            ...
            for formacao in registro.listaFormacaoAcademica:
                xmlTemp += "        <formacao>\n"
                xmlTemp += (
                    "          <ano_inicio>" + formacao.anoInicio + "</ano_inicio>\n"
                )
                xmlTemp += (
                    "          <ano_conclusao>"
                ...
                xmlTemp += (
                    "          <nome_instituicao>"
                    + formacao.nomeInstituicao
                    + "</nome_instituicao>\n"
                )
                xmlTemp += (
                    "          <descricao>" + formacao.descricao + "</descricao>\n"
                )
                xmlTemp += "        </formacao>\n"
            xmlTemp += "    </formacao_academica>\n"
        return xmlTemp
    ...
    def getProjetosPesquisa(self, registro):
        ...
        if registro.listaProjetoDePesquisa:
            ...
            for pesquisa in registro.listaProjetoDePesquisa:
                ...
                xmlTemp += (
                    "          <ano_inicio>"
                    + str(pesquisa.anoInicio)
                    + "</ano_inicio>\n"
                )
                xmlTemp += (
                    "          <ano_conclusao>"
                    + str(pesquisa.anoConclusao)
                    + "</ano_conclusao>\n"
                )
    ...
    def getProjetosExtensao(self, registro):
        xmlTemp = ""
        if registro.listaProjetoDeExtensao:
            xmlTemp += "    <projetos_extensao>\n"
            for extensao in registro.listaProjetoDeExtensao:
                xmlTemp += "        <projeto>\n"
                xmlTemp += (
                    "          <ano_inicio>"
                    + str(extensao.anoInicio)
                    + "</ano_inicio>\n"
                )
                xmlTemp += (
                    "          <ano_conclusao>"
                    + str(extensao.anoConclusao)
                    + "</ano_conclusao>\n"
                )
                xmlTemp += "          <nome>" + extensao.nome + "</nome>\n"
                descricaoTmp = ""
                for item in extensao.descricao:
                    descricaoTmp += item  # <--
                xmlTemp += "          <descricao>" + descricaoTmp + "</descricao>\n"
                xmlTemp += "        </projeto>\n"
            xmlTemp += "    </projetos_extensao>\n"
    ...
    def getOutrosProjetos(self, registro):
        ...
        if registro.listaOutrosProjetos:
            xmlTemp += "    <outros_projetos>\n"
            for outroProjeto in registro.listaOutrosProjetos:
                xmlTemp += "        <projeto>\n"
                xmlTemp += (
                    "          <ano_inicio>"
                    + str(outroProjeto.anoInicio)
                    + "</ano_inicio>\n"
                )
                xmlTemp += (
                    "          <ano_conclusao>"
                    + str(outroProjeto.anoConclusao)
                    + "</ano_conclusao>\n"
                )
                xmlTemp += "          <nome>" + outroProjeto.nome + "</nome>\n"
                descricaoTmp = ""
                for item in outroProjeto.descricao:
                    descricaoTmp += item
                xmlTemp += "          <descricao>" + descricaoTmp + "</descricao>\n"
                xmlTemp += "        </projeto>\n"
            xmlTemp += "    </outros_projetos>\n"
    ...
    def getAreaDeAtuacao(self, registro):
        ...
        if registro.listaAreaDeAtuacao:
            ...
            for atuacao in registro.listaAreaDeAtuacao:
                xmlTemp += (
                    "          <descricao>" + atuacao.descricao + "</descricao>\n"
                )
            xmlTemp += "    </area_atuacao>\n"
        return xmlTemp
